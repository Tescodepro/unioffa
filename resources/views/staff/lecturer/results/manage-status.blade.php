        ...
        <div class="page-wrapper">
            <div class="content">
                            ...
                            <div class="col-md-2">
                                ...
                                <select name="level" class="form-select" required>
                                    ...
                                    @foreach ($levels as $lvl)
                                        <option value="{{ $lvl }}"
                                            {{ request('level') == $lvl ? 'selected' : '' }}>{{ $lvl }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-2">
                                ...
                                <select name="session" class="form-select" required>
                                    ...
                                    @foreach ($sessions as $sess)
                                        <option value="{{ $sess }}"
                                            {{ request('session') == $sess ? 'selected' : '' }}>{{ $sess }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                                ...
                                <select name="semester" class="form-select" required>
                                    ...
                                    <option value="1st" {{ request('semester') == '1st' ? 'selected' : '' }}>First
                                        Semester</option>
                                    <option value="2nd" {{ request('semester') == '2nd' ? 'selected' : '' }}>Second
                                        Semester</option>
                                </select>
                            </div>
                ...
                @if (request()->filled('department_id') && count($records) > 0)
                                        ...
                                        <tbody>
                                            @foreach ($records as $row)
                                                <tr>
                                                    ...
                                                    <td>{{ $row->name }}</td>
                                                    <td>{{ $row->matric_no }}</td>
                                                    <td>{{ $row->session }}<br><small
                                                            class="text-muted">{{ $row->semester }}</small></td>
                    ...
                    @foreach ($records as $row)
                        <div class="modal fade" id="statusModal{{ str_replace('/', '', $row->matric_no) }}"
                            tabindex="-1">
                                ...
                                <div class="modal-content">
                                    ...
                                    <form action="{{ route('results.update.status') }}" method="POST">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <p>Update status for all courses in {{ $row->semester }} {{ $row->session }}?
                                            </p>
                                                ...
                                                <select name="status" class="form-select" required>
                                                    ...
                                                    @else
                                                        <option value="recommended"
                                                            {{ $row->current_status == 'recommended' ? 'selected' : '' }}>
                                                            Recommended</option>
                                                    @endif
                                                </select>
                                            </div>
