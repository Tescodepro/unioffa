...
@section('title', 'Edit Payment Lockdown Setting')
        ...
        <div class="page-wrapper">
            <div class="content container-fluid">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h4 class="mb-0 fw-bold">Edit Payment Lockdown Setting</h4>
                        <p class="text-muted small mb-0">Update the deadline to lock the payment portal for targeted students</p>
                    </div>
                ...
                <form action="{{ route('bursary.payment-lockdown-settings.update', $lockdown->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-lg-8">
                            <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
                                <div class="card-header bg-white border-0 py-3 d-flex align-items-center">
                                    ...
                                    <h6 class="mb-0 fw-bold">Update Lockdown Rules</h6>
                                </div>
                                    ...
                                    <div class="row g-3">
                                        <div class="col-md-12">
                                            ...
                                            <input type="text" name="title" class="form-control form-control-lg border-0 bg-light @error('title') is-invalid @enderror" 
                                                placeholder="e.g. 2025/2026 First Semester Tuition Portal Closure" value="{{ old('title', $lockdown->title) }}" required>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="datetime-local" name="deadline" class="form-control form-control-lg border-0 bg-light @error('deadline') is-invalid @enderror" 
                                                value="{{ old('deadline', $lockdown->deadline ? \Carbon\Carbon::parse($lockdown->deadline)->format('Y-m-d\TH:i') : '') }}" required>
                                        ...
                                        <div class="col-12 mt-4">
                                            <div class="form-check form-switch bg-light p-3 rounded-3 d-flex align-items-center justify-content-between">
                                                ...
                                                <input class="form-check-input fs-4" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $lockdown->is_active) ? 'checked' : '' }}>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-lg-4">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="campus_ids[]" class="form-select select2-multi" multiple="multiple">
                                            @foreach ($campuses as $campus)
                                                <option value="{{ $campus->id }}" {{ in_array($campus->id, old('campus_ids', is_array($lockdown->campus_ids) ? $lockdown->campus_ids : [])) ? 'selected' : '' }}>
                                                    {{ $campus->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="faculty_ids[]" class="form-select select2-multi" multiple="multiple">
                                            @foreach ($faculties as $faculty)
                                                <option value="{{ $faculty->id }}" {{ in_array($faculty->id, old('faculty_ids', is_array($lockdown->faculty_ids) ? $lockdown->faculty_ids : [])) ? 'selected' : '' }}>
                                                    {{ $faculty->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="department_ids[]" class="form-select select2-multi" multiple="multiple">
                                            @foreach ($departments as $dept)
                                                <option value="{{ $dept->id }}" {{ in_array($dept->id, old('department_ids', is_array($lockdown->department_ids) ? $lockdown->department_ids : [])) ? 'selected' : '' }}>
                                                    {{ $dept->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="levels[]" class="form-select select2-multi" multiple="multiple">
                                            @foreach ($levels as $lvl)
                                                <option value="{{ $lvl }}" {{ in_array($lvl, old('levels', is_array($lockdown->levels) ? $lockdown->levels : [])) ? 'selected' : '' }}>
                                                    Level {{ $lvl }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="admission_sessions[]" class="form-select select2-multi" multiple="multiple">
                                            @foreach ($sessions as $session)
                                                <option value="{{ $session }}" {{ in_array($session, old('admission_sessions', is_array($lockdown->admission_sessions) ? $lockdown->admission_sessions : [])) ? 'selected' : '' }}>
                                                    {{ $session }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="genders[]" class="form-select select2-multi" multiple="multiple">
                                            @foreach ($genders as $gender)
                                                <option value="{{ $gender }}" {{ in_array($gender, old('genders', is_array($lockdown->genders) ? $lockdown->genders : [])) ? 'selected' : '' }}>
                                                    {{ $gender }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="entry_modes[]" class="form-select select2-multi" multiple="multiple">
                                            @foreach ($entryModes as $mode)
                                                <option value="{{ $mode->code }}" {{ in_array($mode->code, old('entry_modes', is_array($lockdown->entry_modes) ? $lockdown->entry_modes : [])) ? 'selected' : '' }}>
                                                    {{ $mode->code }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="programmes[]" class="form-select select2-multi" multiple="multiple">
                                            @foreach ($programmes as $programme)
                                                <option value="{{ $programme }}" {{ in_array($programme, old('programmes', is_array($lockdown->programmes) ? $lockdown->programmes : [])) ? 'selected' : '' }}>
                                                    {{ $programme }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <button type="submit" class="btn btn-dark w-100 py-3 shadow-sm rounded-3">
                                <i class="ti ti-device-floppy me-2"></i> Update Lockdown Rule
                            </button>
                        </div>
                    </div>
                </form>
